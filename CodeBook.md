...
Variables of 'tidy.txt' are derived from the "Human Activity Recognition Using Smartphones Data Set" which was originally made avaiable here: 
...
A large set of variables were estimated from these signals in the original data, but for this project only following were kept in the data: 
mean(): Mean value
...
The resulting measurement variable names differ somewhat from the original form in order to use appropriate labels. Measurement variables have the following form: tBodyAccmeanX, which means the mean value of tBodyAcc-XYZ.
